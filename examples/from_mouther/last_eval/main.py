...
@app.get("/history/{input_str}", response_model=List[HistoryResponse])
async def get_history_by_string(input_str: str):
    """
    GET endpoint to retrieve the history for a specific input string.
    """
    logger.info(f"GET request for history of string: '{input_str}'")
    query = History.__table__.select().where(History.input_str == input_str)
    return await database.fetch_all(query)
