    ...
    while attempt < retries:
        ...
        try:
            ...
            if "routes" in data and len(data["routes"]) > 0:
                ...
                send_slack_message(f"✅ Route found from {origin} to {destination}!")
                # Save JSON response
                ...
                send_slack_message(f"⚠️ Warning: No routes found from {origin} to {destination}. Retrying...")
                time.sleep(wait_sec)

        except requests.exceptions.RequestException as e:
            logger.error(f"Request failed: {e}", exc_info=True)
            send_slack_message(f"❌ Error: Failed to call Google Maps API: {e}. Retrying...")
            time.sleep(wait_sec)
        except json.JSONDecodeError as e:
            logger.error(f"Failed to parse JSON: {e}", exc_info=True)
            send_slack_message(f"❌ Error: Failed to parse response: {e}. Retrying...")
            time.sleep(wait_sec)
    else:
        logger.error(f"All {retries} attempts failed for {origin} to {destination}")
        send_slack_message(f"❌ Error: All {retries} attempts failed for route from {origin} to {destination}")
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("Logging_im:app", host="127.0.0.1", port=8000, reload=True)